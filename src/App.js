...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (isMobile()) {
      window.addEventListener("click", addChar)
      return () => {
          window.removeEventListener("click", addChar)        
      }
    } else {
      window.addEventListener("keyup", addChar)
      return () => {
          window.removeEventListener("keyup", addChar)        
      }
    }
  }, [keystrokeCount, addChar, toggleCursorVisiblity])
